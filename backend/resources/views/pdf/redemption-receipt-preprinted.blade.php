...
<head>
    <meta charset="utf-8">
    <title>Resit Penebusan - {{ $redemption->redemption_no }}</title>
    <style>
        @page {
            size: A5 landscape;
            margin: 0;
        ...
        * {
        ...
        /* ═══ FINANCIAL BREAKDOWN (replaces rate section for redemption) ═══ */
        .finance-section {
            padding: 2mm;
        }
        .finance-title {
...
<body>
    @php
        ...
        $catatan = "TEBUS; Asal: {$pledge->pledge_no}; Pokok: RM " . number_format($principal, 2) . "; Faedah: RM " . number_format($interestAmount, 2) . "; Jumlah: RM " . number_format($totalPaid, 2);

        // Amount in words (total paid)
        $amountWords = '';
        try {
    ...
    <div class="page">
        ...
        <!-- TRANSACTION TYPE BANNER -->
        <div class="type-banner">PENEBUSAN / REDEMPTION</div>
            ...
            <tr>
                ...
                <td class="right-col">
                    <div class="ticket-box">
                        ...
                        <div class="ticket-number">{{ $redemption->redemption_no }}</div>
                    </div>
                    <!-- Financial Breakdown for Redemption -->
                    <div class="finance-section">
                        <div class="finance-title">BUTIRAN BAYARAN</div>
                        <div class="finance-row">
                            <span class="finance-label">Pokok / Principal:</span>
                            <span class="finance-val">{{ number_format($principal, 2) }}</span>
                        </div>
                        <div class="finance-row">
                            <span class="finance-label">Faedah / Interest:</span>
                            <span class="finance-val">{{ number_format($interestAmount, 2) }}</span>
                        </div>
                        @if($handlingFee > 0)
                        <div class="finance-row">
                            <span class="finance-label">Caj Pengendalian:</span>
                            <span class="finance-val">{{ number_format($handlingFee, 2) }}</span>
                        </div>
                        @endif
                        <div class="finance-row-total">
                            <span class="finance-label">JUMLAH / TOTAL:</span>
                            <span class="finance-val">RM {{ number_format($totalPaid, 2) }}</span>
                        </div>
                    </div>
                </td>
            </tr>
        ...
        <div class="amount-row">
            ...
            <span class="amount-words">{{ $amountWords }} RINGGIT DAN {{ strtoupper(number_format(($totalPaid - floor($totalPaid)) * 100)) }} SEN SAHAJA</span>
        </div>
        ...
        <table style="width: 100%; margin-top: 1mm;">
            <tr>
                <td style="font-size: 6px; color: #1a4a7a; vertical-align: bottom;">
                    <div>Sila semak semua barang anda sebelum meninggalkan premis. Tiada tuntutan diterima selepas itu.</div>
                    <div class="status-badge">DITEBUS / REDEEMED</div>
                </td>
