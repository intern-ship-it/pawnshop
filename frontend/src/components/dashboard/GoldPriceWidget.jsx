 ...
 * 
 * Displays live gold prices from MetalPriceAPI
...
export default function GoldPriceWidget({ className = '' }) {
  ...
  const changeColor = change?.direction === 'up' ? 'text-green-600' : 
                      change?.direction === 'down' ? 'text-red-600' : 'text-zinc-500'

  return (
    <div className={`bg-gradient-to-br from-amber-50 to-amber-100 rounded-lg border border-amber-200 p-4 ${className}`}>
      {/* Header */}
      <div className="flex items-center justify-between mb-3">
        ...
        >
          <RefreshCw className={`w-4 h-4 text-amber-700 ${refreshing ? 'animate-spin' : ''}`} />
        </button>
      </div>
        ...
        {change?.amount !== null && (
          <div className={`flex items-center gap-1 mt-1 ${changeColor}`}>
            ...
            <span className="text-xs font-medium">
              {change?.direction === 'up' ? '+' : ''}
              {goldPriceService.formatPrice(change?.amount)} ({change?.percent?.toFixed(2)}%)
            </span>
      ...
      {caratPrices && (
        <div className="border-t border-amber-200 pt-3">
          <p className="text-xs text-zinc-500 mb-2">Price by Purity (per gram)</p>
      ...
      <div className="mt-3 pt-2 border-t border-amber-200 flex items-center justify-between text-xs text-zinc-400">
        <span>
          Source: {source === 'metalpriceapi' ? 'MetalPriceAPI' : 
                   source === 'cache' ? 'Cached' : 
                   source === 'fallback' ? 'Manual' : source}
        </span>
        <span>
          {prices?.last_updated && new Date(prices.last_updated).toLocaleTimeString('en-MY')}
        </span>
      </div>
    </div>
  )
