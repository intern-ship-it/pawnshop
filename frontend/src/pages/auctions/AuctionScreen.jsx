...
export default function AuctionScreen() {
  ...
  useEffect(() => {
    ...
    const hasAuctionData = storedPledges.some(
      (p) =>
        p.status === "overdue" ||
        p.status === "forfeited" ||
        p.status === "auctioned"
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      overdueAmount: overduePledges.reduce(
        (sum, p) => sum + (p.loanAmount || 0),
        0
      ),
      forfeitedCount: forfeitedPledges.length,
      forfeitedValue: forfeitedPledges.reduce(
        (sum, p) => sum + (p.netValue || 0),
        0
      ),
      auctionedCount: auctionedPledges.length,
      auctionedRevenue: auctionedPledges.reduce(
    ...
    if (!price || price <= 0) {
      dispatch(
        addToast({
          type: "error",
          title: "Invalid",
          message: "Please enter a valid auction price",
        })
      );
      return;
    }
    ...
    setTimeout(() => {
      ...
      dispatch(
        addToast({
          type: "success",
          title: "Auctioned",
          message: `Pledge ${selectedPledge.id} sold for ${formatCurrency(
            price
          )}`,
        })
      );
  ...
  return (
    ...
    >
      {/* Dev Warning */}
      <Card className="mb-6 bg-amber-50 border-amber-200 p-4">
        <div className="flex items-center gap-4">
          <div className="p-3 bg-amber-100 rounded-full text-amber-600">
            <Construction className="w-6 h-6" />
          </div>
          <div>
            <h3 className="font-bold text-amber-800">Under Development</h3>
            <p className="text-amber-700 text-sm">
              This module is currently being built. Some features might not be
              fully functional.
            </p>
          </div>
        </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-amber-600">
                  {stats.forfeitedCount}
                </p>
                <p className="text-sm text-zinc-500">
                  {formatCurrency(stats.forfeitedValue)} inventory
                </p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-emerald-600">
                  {stats.auctionedCount}
                </p>
                <p className="text-sm text-zinc-500">
                  {formatCurrency(stats.auctionedRevenue)} revenue
                </p>
              </div>
          ...
          {filteredPledges.length === 0 ? (
            <Card className="p-12 text-center">
              ...
              <p className="text-zinc-500">
                {activeTab === "eligible" &&
                  "No overdue pledges eligible for forfeit"}
            ...
            <div className="space-y-4">
              {filteredPledges.map((pledge, index) => {
                ...
                const status =
                  statusConfig[pledge.status] || statusConfig.overdue;
                          ...
                          {pledge.status === "auctioned" && (
                            <div className="text-right">
                              ...
                              <p className="font-bold text-amber-600">
                                {formatCurrency(pledge.auctionPrice)}
                              </p>
                            </div>
                          )}
                        </div>
          ...
          <div className="flex items-center gap-4 p-4 bg-red-50 border border-red-200 rounded-xl mb-4">
            ...
            <div>
              ...
              <p className="text-sm text-red-600">
                The pledge will be marked as forfeited and items will be
            ...
            <div className="space-y-3 mb-6">
              ...
              <div className="flex justify-between p-3 bg-zinc-50 rounded-lg">
                ...
                <span className="font-semibold">
                  {formatCurrency(selectedPledge.loanAmount)}
                </span>
              </div>
              <div className="flex justify-between p-3 bg-zinc-50 rounded-lg">
                <span className="text-zinc-500">Items</span>
                <span>
                  {selectedPledge.items?.length} item(s) -{" "}
                  {selectedPledge.totalWeight?.toFixed(2)}g
                </span>
              </div>
              <div className="flex justify-between p-3 bg-amber-50 rounded-lg">
                <span className="text-amber-700">Days Overdue</span>
                <span className="font-bold text-amber-600">
                  {getDaysOverdue(selectedPledge.dueDate)} days
                </span>
              </div>
            </div>
          )}
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                  ...
                  <div>
                    ...
                    <span className="ml-2 font-semibold text-emerald-600">
                      {formatCurrency(selectedPledge.netValue)}
                    </span>
                  </div>
                </div>
              </div>
